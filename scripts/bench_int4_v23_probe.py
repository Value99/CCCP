...
#pragma unroll
    ...
    unsigned long long t_ld = 0, t_fma = 0, t_pre = 0;
...
#pragma unroll
    ...
    if (probe) t_fma = clock64();
    if (probe && dbg != nullptr) {
        dbg[0] = t_ld - t_pre;
        dbg[1] = t_fma - t_ld;
    }
...
"
    ...
    cuda_sources=SRC,
    functions=["repack19", "int4_gemv_v19", "v21_probe"],
...
def main() -> None:
    ...
    try:
        p = ext.v21_probe()
        print(f"probe cycles: load={p[0]} compute={p[1]} load占比={p[0]*100//(p[0]+p[1]+1)}%")
