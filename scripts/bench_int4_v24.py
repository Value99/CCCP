    ...
    const int slices)
{
    ...
    // v24: pure half2 math — per byte: 2 cvt + 1 hmul2(scale) + 1 hfma2.
    __half2 acc2[16];
#pragma unroll
...
#pragma unroll
            for (int b = 0; b < 4; ++b) {
                const __half2 vals = __halves2half2(
                    __float2half_rn(
                        static_cast<float>(static_cast<int>(bytes[b] & 15) - 8)),
                    __float2half_rn(
                        static_cast<float>(static_cast<int>(bytes[b] >> 4) - 8)));
                acc2[s2] = __hfma2(
                    __hmul2(vals, sc2v),
                    sx2[(ca + 2 * i + b * 8) >> 1],
                    acc2[s2]);
...
#pragma unroll
    for (int u = 0; u < 16; ++u)
        acc += __low2float(acc2[u]) + __high2float(acc2[u]);
#pragma unroll
