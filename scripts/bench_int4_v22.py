    ...
    const int slices)
{
    ...
    __half* const lut = reinterpret_cast<__half*>(sx) + V19_SLICE;
    ...
    if (here <= 0) return;
    __half* sxh = reinterpret_cast<__half*>(sx);
    for (int c = threadIdx.x; c < here; c += 128) {
        sxh[c] = __float2half_rn(x[k0 + c]);
    }
    ...
    // Row LUT: lut[j][n] = (n - 8) * scale(row_j, slice), precomputed so the
    // main loop replaces per-nibble I2FP+FMUL with one LDS + HFMA2.
    if (lane < 8) {
        const int rj = lane;
        for (int n = 0; n < 16; ++n) {
            lut[warp * 256 + rj * 16 + n] =
                __float2half_rn(static_cast<float>(n) - 8.f);
        }
    }
    __syncwarp();
    ...
    // v22: per-nibble row LUT + half2 math.  Each byte contributes
    // LUT[lo], LUT[hi] (already scaled halves) FMA'd against sx pairs.
    __half2 acc2[16];
#pragma unroll
    for (int u = 0; u < 16; ++u) acc2[u] = __float2half2_rn(0.f);
    const __half* mylut = lut + warp * 256 + j * 16;
    const __half2* sx2 = reinterpret_cast<const __half2*>(sxh);
    const int ss = here >> 5;
...
#pragma unroll
        for (int s2 = 0; s2 < 16; ++s2) {
            ...
            const int ca = ts << 5;
            const __half2 sc2v = __half2half2(
                __ldg(srow + ((k0 + ca + 2 * i) >> 6)));
            const uint32_t w = wv[s2];
            const uint8_t* bytes = reinterpret_cast<const uint8_t*>(&w);
#pragma unroll
            for (int b = 0; b < 4; ++b) {
                const __half vlo = mylut[bytes[b] & 15];
                const __half vhi = mylut[bytes[b] >> 4];
                const __half2 scaled = __hmul2(
                    __halves2half2(vlo, vhi), sc2v);
                const __half2 xpair = sx2[(ca + 2 * i + b * 8) >> 1];
                acc2[s2] = __hfma2(scaled, xpair, acc2[s2]);
            }
        }
    }
    for (int ts = sg * 4; ts < ss; ++ts) {
        const uint32_t w = base[(ts << 5) + lane];
        const int ca = ts << 5;
        const uint8_t* bytes = reinterpret_cast<const uint8_t*>(&w);
        const __half2 sc2v = __half2half2(
            __ldg(srow + ((k0 + ca + 2 * i) >> 6)));
#pragma unroll
        for (int b = 0; b < 4; ++b) {
            ...
            const __half2 xpair = sx2[(ca + 2 * i + b * 8) >> 1];
            acc2[0] = __hfma2(
                __hmul2(__halves2half2(vlo, vhi), sc2v), xpair, acc2[0]);
        }
    }
    float acc = 0.f;
#pragma unroll
    for (int u = 0; u < 16; ++u) acc += __low2float(acc2[u]) + __high2float(acc2[u]);
#pragma unroll
    ...
    int64_t groups)
{
    ...
    int4_gemv_v19_kernel<<<grid, 128, V19_SLICE * sizeof(float) + 4 * 256 * sizeof(__half), stream>>>(
        x.data_ptr<float>(),
