...
This model-independent pool is the multi-GPU counterpart of
:mod:`cccp.packed_hybrid`.  The full
...
def tensor_shard_host_expert(
    ...
    *,
    ...
    for projection, weight in enumerate(expert):
        is_down = projection == len(expert) - 1
        if len(expert) == 3:
            raw, blocks = ResidentRoutedVQPool._tensor_shard_projection_raw(
                weight,
            ...
            raw, blocks = ResidentRoutedVQPool._tensor_shard_raw(
                weight,
...
class TensorHybridRoutedVQPool(PackedHybridPool):
