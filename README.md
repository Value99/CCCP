...
### [👉 GitHub Release 下载页（推荐）](https://github.com/Value99/CCCP/releases/tag/v0.9.16)
...
然后双击 `CCCP-Launcher-0.9.16-Offline-Setup.exe`。安装器会自动校验、合并、解压并启动。模型不包含在启动器发行包内，需要单独下载并放入解压目录的 `models` 文件夹。
...
1. 从上方 [GitHub Release](https://github.com/Value99/CCCP/releases/tag/v0.9.16) 下载全部 6 个离线安装文件。
2. 双击 `CCCP-Launcher-0.9.16-Offline-Setup.exe`，等待校验、合并和解压完成；不要直接在分卷内运行。
...
| Linux · NVIDIA H20-3e · TP1 | 随包 CUDA 13，4096-token Prefill / 4096 上下文 Decode | **Qwen 2462.68/56.94、DSV4 2786.87/46.23 token/s**；均通过 0.9.16 性能与有限值门禁 |
| NVIDIA RTX 5090 | DeepSeek-V4 和 GLM-5.2 的 CUDA/RAM 路径实机回归 | **引擎实测通过** |
| NVIDIA H20-3e · 多并行协议 | GLM-5.2 TP4、GLM Flash S TP1、GLM Flash M TP2、Kimi TP4（GPU 2/3/4/5），4096-token Prefill / 4096 上下文 Decode | **GLM-5.2 932.36/25.48、GLM S 1504.67/30.02、GLM M 1282.44/20.22、Kimi 672.04/15.69 token/s**；均通过 0.9.16 性能与有限值门禁 |
...
- 下载：[GitHub Release v0.9.16](https://github.com/Value99/CCCP/releases/tag/v0.9.16)
