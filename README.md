...
1. 从上方 [GitHub Release](https://github.com/Value99/CCCP/releases/tag/v0.9.10) 下载全部 6 个离线安装文件。
2. 双击 `CCCP-Launcher-0.9.10-Offline-Setup.exe`，等待校验、合并和解压完成；不要直接在分卷内运行。
...
| Linux · NVIDIA H20 单卡 | Qwen3.5 27B Dense VQ 完整加载、有限值、MTP Top-3 与性能 | **0.9.3 真机通过：有效 Decode 93.21 token/s，完整请求 82.39 token/s；0.9.10 未改该路径** |
...
| 双路 CPU 服务器（96 物理核） | Qwen3.5 27B Dense VQ、Q4 NUMA 分片与 64-token Decode | **0.9.10 实测 9.77 token/s；较稳定基线约提升 38.6%，不承诺 30 token/s** |
...
- 下载：[GitHub Release v0.9.10](https://github.com/Value99/CCCP/releases/tag/v0.9.10)
